...
class LoginController extends Controller
{
    ...
    public function do_login(Request $request){
        ...
        if ($info) {
            if ($info['pwd'] == $data['pwd']) {
                ...
                //添加数据库
                UserModel::where(['u_id'=>$info['u_id']])->update(['error_num'=>0,'error_time'=>0,'session_id'=>$sessionid,'login_time'=>$login_time,'pwd'=>$pwd]);
    ...
    public static function update_time()
    {
        ...
        $u_id = $all->u_id;
        // dd($u_id);
        $login_time = UserModel::where(['u_id'=>$u_id])->update(['login_time'=>time() + 1200 ]);
        // dd($login_time);
    ...
    public function index(Request $request)
    {
        ...
        {
            echo '<script>alert("你的账号已在其他地方登录,请求下线");location.href = "http://www.lavl.com/test/login"</script>';
    ...
    {
        $status = request('status');//二维码标识
        $openid = Cache::get('wx_'.$status);
        if($openid){
